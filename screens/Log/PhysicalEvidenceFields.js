...
import { Button, Input, Item, Icon, Text } from 'native-base';
...
export default class PhysicalEvidenceFields extends Component {
  ...
  focusLatestField = () => {
    ...
    const fieldRef = this[lastFieldName];
    ...
    return (
      <>
        <Item onPress={this.handleAddNewRow}>
          <TouchableOpacity>
            <Button transparent primary full onPress={this.handleAddNewRow}>
              <Text>Add more evidence</Text>
            </Button>
          </TouchableOpacity>
        </Item>
        {this.renderRows()}
